...
import { call, put, take, takeEvery, all, select, race } from "redux-saga/effects";
...
function* resetKYCSaga() {
  let login = yield select(state => state.login);
  try {
    const jumio = yield call(
      rsf.firestore.deleteDocument,
      `jumio/${login.user.uid}`
    );
    const comply = yield call(
      rsf.firestore.deleteDocument,
      `comply/${login.user.uid}`
    );
    const status = yield call(
      rsf.firestore.deleteDocument,
      `status/${login.user.uid}`
    );
    yield put(push("/jumio"));
  } catch (error) {
    console.log(error);
